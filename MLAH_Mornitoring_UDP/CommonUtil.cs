...
public  class CommonUtil
{
    ...
    /// <summary>
    /// 빅 엔디안 바이트 배열의 특정 위치에서 4바이트를 읽어 Signed Integer(int)로 변환합니다.
    /// </summary>
    /// <param name="buffer">소스 바이트 배열</param>
    /// <param name="startIndex">읽기 시작할 인덱스</param>
    ...
    public static int ReadInt32BigEndian(byte[] buffer, int startIndex)
    {
        ...
        // int는 부호가 있지만, 비트 구성 방식은 2의 보수법을 따르므로 
        // uint와 동일하게 비트 OR 연산으로 합친 뒤 int로 반환하면 됩니다.
        // (C#에서 byte << 24 연산 결과는 자동으로 int가 됩니다)
