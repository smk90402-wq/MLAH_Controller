...
namespace MLAH_LogAnalyzer
{
    ...
    /// </summary>
    public partial class View_Unit_Map_Target : UserControl
    {
        private static View_Unit_Map_Target _instance;

        public static View_Unit_Map_Target SingletonInstance
        {
            get
            {
                if (_instance == null)
                    throw new InvalidOperationException("View_Unit_Map이 아직 생성되지 않았습니다.");
                return _instance;
            }
        }
        public View_Unit_Map_Target()
        {
        ...
        //FPS 체크
        private readonly TimeSpan _mouseMoveInterval = TimeSpan.FromMilliseconds(33); 
        ...
        private void MapControl_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            if (!isFocusModeActive)
            {
                ...
                if (cp == null) return;
                ViewModel_Unit_Map_Target.SingletonInstance.MapCursorLat = cp.GetY();
                ViewModel_Unit_Map_Target.SingletonInstance.MapCursorLon = cp.GetX();
                return;
            }
